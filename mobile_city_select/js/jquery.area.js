...
/*定义三级省市区数据*/
var province = 	["北京市", "天津市", "河北省", "河南省", "山西省", "山东省", "内蒙古自治区", "辽宁省", "吉林省", "黑龙江省", "上海市", "江苏省", "浙江省", "福建省", "江西省", "安徽省", "湖北省", "湖南省", "广东省", "广西壮族自治区", "海南省", "重庆市", "四川省", "贵州省", "云南省", "西藏自治区", "陕西省", "甘肃省", "青海省", "宁夏回族自治区", "新疆维吾尔自治区", "台湾省", "香港", "澳门"];
var city = 		[
					["市辖区"],
					["市辖区", "市辖县"],
...
function intProvince() {
	...
	for (var i=0; i<province.length; i++) {
	    areaCont += '<li onClick="setPhoneAddress(' + i + ');">' + province[i] + '</li>';
	}
...
/*选择省份*/
function selectP(p,callback) {
	areaCont = "";
	areaList.html("");
	for (var j=0; j<city[p].length; j++) {
	    areaCont += '<li onClick="setPhoneAddress(' + p + ',' + j + ');">' + city[p][j] + '</li>';
	}
	areaList.html(areaCont);
	$("#areaBox").scrollTop(0);
	if (province[p] == "澳门" || province[p] == "香港" || province[p] == "台湾省") {
	    expressArea = province[p];
	    $("#expressArea input").val(expressArea);
	    clockArea();
	    callback({ "p": province[p], "c": "", "d": "" });
	} else {
	    expressArea = province[p] + ">";
...
/*选择城市*/
function selectC(p,c,callback) {
	areaCont = "";
	for (var k=0; k<district[p][c].length; k++) {
		areaCont += '<li onClick="setPhoneAddress(' + p + ',' + c + ',' + k + ');">' + district[p][c][k] + '</li>';
	}
	areaList.html(areaCont);
	$("#areaBox").scrollTop(0);
	var sCity = city[p][c];
	if (sCity != "省直辖县级行政单位") {
		if (sCity == "东莞市" || sCity == "中山市" || sCity == "儋州市" || sCity == "嘉峪关市") {
			...
			$("#expressArea input").val(expressArea);
			clockArea();
			callback({ "p": province[p], "c": city[p][c], "d": "" });
...
/*选择区县*/
function selectD(p, c, d,callback) {
	clockArea();
	expressArea += district[p][c][d];
	$("#expressArea input").val(expressArea);
	callback({ "p": province[p], "c": city[p][c], "d": district[p][c][d] });
}
