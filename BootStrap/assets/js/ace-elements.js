...
(function($ , undefined) {
	...
	var Ace_File_Input = function(element , settings) {
		...
		this.$element.wrap('<' + tagName + ' class="ace-file-input" style="width:' + this.settings.thumbnail + 'px;"/>');
	...
	Ace_File_Input.prototype.apply_settings = function() {
		...
		this.$element.after('<span class="ace-file-container" data-title="' + this.settings.btn_choose + '" style=" width:' + this.settings.thumbnail + 'px; height:' + this.settings.thumbnail + 'px;"><span class="ace-file-name" data-title="' + this.settings.no_file + '">' + (this.settings.no_icon ? '<i class="' + ace.vars['icon'] + this.settings.no_icon + '"></i>' : '') + '</span></span>');
		this.$label = this.$element.next();                                                             //gesheng update 去掉上面span的边宽高 2015-8-8 11:35:28 
		...
		if(remove_btn) {
			...
			btn.on(ace.click_event, function(e){
			    e.preventDefault();
			    $("#form-inputfile .set").text("选择文件");
				if( !self.can_reset ) return false;
				$(this).prev().css("border", "1px solid #ccc");        //gesheng  update   点击取消时加上边框  2015-8-8 13:59:59
				var ret = true;
	...
	var preview_image = function(file) {
		...
		var imgLoaded = function(img) {
		    ...
		    var size = 50;
		    $span.parent().css("border", "0px");             //gesheng  update   预览图片时取消边框  2015-8-8 13:59:59
			if (self.settings.thumbnail == 'large') size = 150;
			    //else if(self.settings.thumbnail == 'fit') size = $span.width();
			else { size = self.settings.thumbnail }             //gesheng update 2015-8-8 11:33:06
			...
			$(img).css({'background-image':'url('+thumb.src+')' , width:w, height:h-3})
	...
	$.fn.ace_file_input.defaults = {
	    ...
	    //no_icon:'',            //gesheng update 2015-8-8 11:52:40 
		btn_choose: 'Choose',
		btn_change: 'Change',
		icon_remove: 'iconTags iconAbolish',
		droppable: false,
...
(function($ , undefined) {
	...
	$.fn.aceTree = $.fn.ace_tree = function(options) {
		var $defaults = {
			...
			'unselected-icon': ace.vars['icon'] + 'iconTags iconAbolish',
			'loadingHTML': 'Loading...'
		}
